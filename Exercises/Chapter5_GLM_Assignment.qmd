...
formulas <- list(model1 = formula(bee ~ forest.scaled + altitude.scaled + 
                                    land.use.scaled + temperature.scaled  + 
                                    (1|method) + (1|effort)), 
                 model2 = formula(bee ~ forest.scaled + altitude.scaled + 
                                    land.use.scaled + (1|method) + (1|effort)), 
                 model3 = formula(bee ~ forest.scaled + altitude.scaled + 
                                    (1|method) + (1|effort)), 
                 model4 = formula(bee ~ forest.scaled + (1|method) + 
                                    (1|effort)))
...
aictable_pretty <- aictable %>% tibble() %>%
  ...
  mutate(Model = case_when(Model == 'model1' ~ 'forest + altitude + 
                           land.use + temperature', 
                           Model == 'model2' ~ 'forest + altitude + land.use', 
                           Model == 'model3' ~ 'forest + altitude', 
                           Model == 'model4' ~ 'forest'), 
...
In more detail, abundance data of *Eulaema nigrita* as independent variable was modeled using a GLMM from the poisson family with a log-link function. Four different dependent variables were utilised to specify environment-abundance relationship: 
  ...
  Furthermore, I considered information on the monitoring method (nets, traps or both) and the monitoring effort (measures as time spent in each plot, transferred by taking the log) in the random effects. Thereby, I accounted for difference efficiencies in the applied monitoring methods and varying monitoring efforts, respectively, and reduced the bias due to imperfect detection.

Overall, I fitted four candidate models where all four environmental covariates where added step wise in an additive manner to built up from simple to more complex models. Each of these candidate models was compared using the AICc (small sample size corrected Aikaile Information Criterion - approaches the classical AIC for large sample sizes and is therefore generally recommended over the AIC) to find the most parsimonious model. However, I retained the correction for imperfect detection with two crossed random effects (survey method and effort) for all candidate models since I assumed both variables to be important for explaining the observation process. Afterwards, I employed the (set of) model(s) for inference to extract information on the abundance-environmental covariate relationship as a) effect sizes and b) marginal effect plots of each independent variable found to be important.
...
All inference was solely based on the single best model since I found overwhelming support for this full model indicated by the delta AICc and AICc weights, for more details see Table 2. No model averaging with other candidate models was needed.
...
#| fig-height: 6
#| fig.align: center
#| fig-cap: "Effect of environmental covariates on predicted Eulaema nigrita abundance. The Abundance was explained using a GLMM framework where only the influence of the fixed effects are shown here but random effects were used to correct for sampling effort and method."

plot
```

I found that both altitude and mean annual temperature are the two striking environmental covariates explaining the species' abundance both showing positive relationships. However the two environmental covariates forest cover and land use heterogeneity were important variables to explain bee abundance (see Table 2), even though their net effect on the abundance was comparably less pronounced (see Table 1). 

Transferring this knowledge into a conservation context, I found that the distribution of *Eulaema nigrita* is largely driven by mean annual temperature and altitude - both factors we cannot (directly) influence. However, forest cover and land use heterogeneity may be subject to human alteration. Thus, out conservation intervention should focus on these by reducing forest cover at the sites where the bee might be a target species and by increasing the land use heterogeneity for example by adopting new, small scale farming methods with high diversity of different land treatments on small space.

## 5. Code Availability

Please find the code on my github page. 
