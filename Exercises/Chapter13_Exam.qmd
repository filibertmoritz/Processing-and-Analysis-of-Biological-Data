...
effect.sizes <- tibble(Parameters =  str_remove(rownames(s$coefficients), 
       ...
       `2.5% confidence level` = s$coefficients[,1] - 1.96*s$coefficients[,2], 
       `97.5% confidence level` = s$coefficients[,1] + 1.96*s$coefficients[,2],
       # `P-value` = s$coefficients[,4]
       ) %>% 
   mutate(across(c('Parameter estimates', '2.5% confidence level',
                   '97.5% confidence level',# 'P-value'
                   ), ~ round(.x, 2)),
          Parameters = case_when(Parameters == 'populationTurkey Hill Rd' ~ 'Population:Turkey Hill Rd', 
                                 ...
                                 .default = Parameters),
          Parameters = str_replace_all(str_to_title(Parameters), pattern = '\\.', ' '))
...
We employed the single best model for inference and extracted information on a) the population specific fitness, and b) the relation between phenotypic traits and fitness. Therefore, we reported effect sizes produced marginal effect plots of each independent variable retained in the best model and thus considered as important for explaining *Penstemon digitalis* fitness. All analysis were performed in R (R Core Team, 2024) and models were fitted using the 'lme4' package (Bates et. al, 2015). 
...
```{r, echo = F, fig.}
#| tbl-cap: "Model selection table for all considered candidate models ordered by minimal AICc (small sample size corrected Aikaike Information criterion)."
aictable_pretty
```

However, effect sizes differed considerably between phenotypic traits: Total floral scent emission had the strongest association with fitness (slope `{r} e.scent` fitness per unit SD (confidence intervals (CI: 2.5 %, 97.5 %): `{r} ci.scent[1]`, `{r} ci.scent[2]`). For example, a change of 50 % in total floral scent emissions of the population 'Neimi Rd' resulted in an fitness increase of `{r} round(difference['pred'],1)` (`{r} round(difference['pred.upper'],1)`, `{r} round(difference['pred.lower'],1)`) %. Flowering date and and plant height had a slightly lower but still notable effect on fitness (with slopes of `{r} e.flowering.date` (`{r} ci.flowering.date[1]`, `{r} ci.flowering.date[2]`) and `{r} e.height` (`{r} ci.height[1]`, `{r} ci.height[2]`) in fitness per unit SD). In contrast, flower size had only little effect on *Penstemon digitalis* fitness (see Table 2 and Figure 1).

The source population was an important predictor for *Penstemon digitalis* fitness. In more detail, the populations 'Whipple Farm'  and 'Neimi Rd' had the highest fitness with values of `{r} round(W.Farm.mean,1) ` (`{r} round(W.Farm.lower,1) `, `{r} round(N.Rd.upper,1)`) and `{r} round(N.Rd.mean,1) ` (`{r} round(N.Rd.lower,1) `, `{r} round(W.Farm.upper,1)`), respectively. In contrast, the donor population 'Turkey Hill Rd' possessed a much lower average fitness of only `{r} round(T.Hill.mean,1) ` (`{r} round(T.Hill.lower,1) `, `{r} round(T.Hill.upper,1)`) (see Table 1). 

```{r, echo = F}
#| tbl-cap: "Parameter estimates of the single best (LMM) model explaining *Penstemon digitalis* fitness by phenotypic traits and source population. All estimates of numeric variables are in units of standard deviations. The most parsimonous model was detected by AICc (see Table 2)"
#| 
effect.sizes.pretty 
```
...
#| fig-height: 7
#| fig.align: center
#| fig-cap: "Effect of phenotypic traits and different source populations on *Penstemon digitalis* fitness in a common garden experiment. Fitness was assessed as a composite measure based on fruit number and weight and analysed in a linear mixed effect model with a the experimental block as random effect to correct noise originating from the experimental set up."
...
We found that the studied populations differ in their fitness, but higher fitness is also associated with high floral scent emissions, larger plants and an earlier flowering date (see Table 2 and Figure 1). 

We caution that the effect of phenotypic traits on fitness might differ by population, however, we did not consider this hypothesis in our candidate model set since too little data was available to fit such a highly parameterised model. Thus, more data and research is needed to facilitate a deeper  understanding of the population specific phenotypic traits driving *Penstemon digitalis* fitness. 

In conclusion, we recommend to select *Penstemon digitalis* individuals from the 'Whipple Farm' and 'Neimi Rd'  populations for breeding that are large, flower early and possess an intense smell - the latter phenotypic trait might not only benefit pollinators but might also have a pleasant side effects for gardeners.
